...
| 痛点 | 解决方案 |
|------|----------|
| 不知道网上有没有现成的 Skill | 5 层搜索源覆盖：SkillsMP → Skills.sh → CocoLoop → GitHub → clawhub |
| 写完 Skill 不知道好不好 | 10 维度评分（120 分制），每个分数有据可查 |
| 下载的 Skill 是否安全 | 4 步安全审查协议：元数据→权限→内容→Typosquat |
| 部署容易出错 | 标准化部署流程，永远用 cp 不用 mv |
...
**搜索降级策略：**
```
SkillsMP API → npx skills find → CocoLoop → GitHub → clawhub → 提示用户
```
...
1. 需求分析 — 功能定位、触发设计、执行模式、输出规范
2. 生成 SKILL.md — 按官方规范生成 frontmatter + 正文
3. 创建目录结构 — 使用 `init_skill.py` 或手动创建
4. 本地测试 — 验收清单 7 项全部通过
5. 打包部署 — 使用 `package_skill.py` 或 cp 到全局
...
4. **验证部署**
   ...
   ```
5. **测试全局可用** — 使用 Agent CLI 测试，如 `claude -p "使用 {name} 完成 XXX"`
...
| 文件 | 说明 |
|------|------|
| `references/official-spec.md` | Agent Skill 规范 |
| `references/template.md` | SKILL.md 模板文件 |
| `references/example.md` | 好结果、坏结果、边界样例 |
| `references/methodology.md` | 社区实战经验总结 |
...
|------|------|
| `references/official-spec.md` | Agent Skill 规范 |
| `references/scoring-criteria.md` | 10 维度评分标准（120 分制） |
...
| 文件 | 说明 |
|------|------|
| `references/optimize.md` | 优化方法论 |
| `references/deploy.md` | 部署指南 |
